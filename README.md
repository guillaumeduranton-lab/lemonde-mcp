...
Claude's mobile app can only use **remote** MCP servers that you add on claude.ai. So the server must run on a public HTTPS host. Access is protected by GitHub OAuth, and only `ALLOWED_GITHUB_LOGIN` is allowed in.

1. **Deploy** (Render free tier, using the included `render.yaml` and `Dockerfile`): on render.com, go to *New* > *Blueprint* and pick this repo. Note the URL it gives you, e.g. `https://lemonde-mcp.onrender.com`.
2. **Create a GitHub OAuth App** at github.com > Settings > Developer settings > OAuth Apps > *New*:
   - Homepage URL: `https://lemonde-mcp.onrender.com`
   - Authorization callback URL: `https://lemonde-mcp.onrender.com/auth/callback`
3. **Set env vars** on Render:
   | Var | Value |
   |---|---|
   | `PUBLIC_URL` | `https://lemonde-mcp.onrender.com` |
   | `ALLOWED_GITHUB_LOGIN` | your GitHub username |
   | `GITHUB_CLIENT_ID` / `GITHUB_CLIENT_SECRET` | from the OAuth App |
   | `LEMONDE_COOKIE` | your lemonde.fr Cookie header |
   | `TYPESAFE_API_KEY` | optional, for `triage_news` |
   | `JWT_SIGNING_KEY` | auto-generated by the blueprint |
4. **Add to Claude**: on claude.ai, go to *Settings* > *Connectors* > *Add custom connector* and enter URL `https://lemonde-mcp.onrender.com/mcp`, then sign in with GitHub. The connector then shows up in the Claude iOS/Android app (enable it in a chat under the tools menu).

Notes: Render's free tier sleeps when idle, so the first call can take about 30 s. When the Le Monde cookie expires, update `LEMONDE_COOKIE` and redeploy.
